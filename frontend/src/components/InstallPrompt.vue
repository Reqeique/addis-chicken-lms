<template>
	<Dialog v-model:open="showDialog">
		<template #title>
			<h2 class="text-lg-bold">{{ __('Install Addis Chicken Training') }}</h2>
		</template>
		<template #default>
			<p>
				{{
			...
			>
				...
				>
					<span class="text-base-bold text-ink-gray-9">
						{{ __('Install Addis Chicken Training') }}
					</span>
...
if (
	isIos() &&
	!isInStandaloneMode() &&
	localStorage.getItem('addisChickenIosInstallPromptShown') !== 'true'
) {
	iosInstallMessage.value = true
	localStorage.setItem('addisChickenIosInstallPromptShown', 'true')
}
